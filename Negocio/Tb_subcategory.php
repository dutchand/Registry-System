...
function findAll() {
	 ...
	 */
	$fields = array('id_subcategory','id_category','descriptionsubcategory');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `tb_subcategory` $where"); 
	...
	$query_recordset = "SELECT tbs.id_subcategory, tbm.id_category, tbm.`descriptioncategory`, tbs.descriptionsubcategory
FROM tb_subcategory tbs , tb_maincategory tbm
WHERE tbs.`id_category` = tbm.`id_category`  $order";
...
function fillallsubs()
		{
			global $conn;
			
			$query_select = "SELECT tbm.id_category, tbm.`descriptioncategory`,tbs.id_subcategory,  tbs.descriptionsubcategory, tbss.`id_subsubcategory`, tbss.`descriptionsubsubcategory`
FROM tb_subcategory tbs , tb_maincategory tbm, tb_subsubcategory tbss
WHERE tbs.`id_category` = tbm.`id_category` AND tbs.`id_subcategory` = tbss.`id_subcategory`";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `tb_subcategory` (id_subcategory,id_category,descriptionsubcategory) VALUES (%s,%s,%s)" ,			
	        GetSQLValueString($_REQUEST["addcode"], "text"), # 
			GetSQLValueString($_REQUEST["id_category"], "text"), # 
			GetSQLValueString($_REQUEST["descriptionsubcategory"], "text")# 
	);
	...
	if ($ok) {
		...
		$toret = array(
			"data" => array(
				array(
					"id_subcategory" => $_REQUEST["id_subcategory"], 
					"id_category" => $_REQUEST["id_category"], # 
					"descriptionsubcategory" => $_REQUEST["descriptionsubcategory"]# 
				)
			), 
			"metadata" => array()
		);
...
function insert2() {
	...
	//build and execute the insert query
	$query_insert = sprintf("INSERT INTO `tb_subsubcategory` (id_subsubcategory,id_subcategory,descriptionsubsubcategory) VALUES (%s,%s,%s)" ,			
	        GetSQLValueString($_REQUEST["shortcode"], "text"), # 
			GetSQLValueString($_REQUEST["id_subcategory"], "text"), # 
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `tb_subcategory` WHERE id_subcategory = %s",
		GetSQLValueString($_REQUEST["id_subcategory"], "text")
	);
	$recordset = mysql_query($query_recordset, $conn);
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `tb_subcategory` SET id_category = %s,descriptionsubcategory = %s WHERE id_subcategory = %s", 
			GetSQLValueString($_REQUEST["id_category"], "text"), 
			GetSQLValueString($_REQUEST["descriptionsubcategory"], "text"), 
			GetSQLValueString($row_recordset["id_subcategory"], "text")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => array(
					array(
						"id_subcategory" => $row_recordset["id_subcategory"], 
						"id_category" => $_REQUEST["id_category"], #
						"descriptionsubcategory" => $_REQUEST["descriptionsubcategory"]#
					)
				), 
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `tb_subcategory` WHERE id_subcategory = %s", 
			GetSQLValueString($row_recordset["id_subcategory"], "text")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => $row_recordset["id_subcategory"], 
				"metadata" => array()
			);
		} else {
			$toret = array(
