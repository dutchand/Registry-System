...
function findAll() {
	 ...
	 */
	$fields = array('id_tbfiledata','id_minfile','refno','fromwhere','subjecttitle','dateadded','filefotoname');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `tb_filedata` $where"); 
	...
	$query_recordset = "SELECT id_tbfiledata,id_minfile,refno,fromwhere,subjecttitle,dateadded,filefotoname FROM `tb_filedata` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `tb_filedata` (id_minfile,refno,fromwhere,subjecttitle,dateadded,filefotoname) VALUES (%s,%s,%s,%s,%s,%s)" ,			GetSQLValueString($_REQUEST["id_minfile"], "text"), # 
			GetSQLValueString($_REQUEST["refno"], "text"), # 
			GetSQLValueString($_REQUEST["from"], "text"), # 
			GetSQLValueString($_REQUEST["subject"], "text"), # 
			GetSQLValueString($_REQUEST["dataadded"], "text"), # 
	...
	if ($ok) {
		...
		$toret = array(
			"data" => array(
				array(
					"id_tbfiledata" => mysql_insert_id(), 
					"id_minfile" => $_REQUEST["id_minfile"], # 
					"refno" => $_REQUEST["refno"], # 
					"from" => $_REQUEST["from"], # 
					"subject" => $_REQUEST["subject"], # 
					"dataadded" => $_REQUEST["dataadded"], # 
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `tb_filedata` WHERE id_tbfiledata = %s",
		GetSQLValueString($_REQUEST["id_tbfiledata"], "int")
	);
	$recordset = mysql_query($query_recordset, $conn);
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `tb_filedata` SET id_minfile = %s,refno = %s,from = %s,subject = %s,dataadded = %s,filename = %s WHERE id_tbfiledata = %s", 
			GetSQLValueString($_REQUEST["id_minfile"], "text"), 
			GetSQLValueString($_REQUEST["refno"], "text"), 
			GetSQLValueString($_REQUEST["from"], "text"), 
			GetSQLValueString($_REQUEST["subject"], "text"), 
			GetSQLValueString($_REQUEST["dataadded"], "text"), 
			GetSQLValueString($_REQUEST["filename"], "text"), 
			GetSQLValueString($row_recordset["id_tbfiledata"], "int")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => array(
					array(
						"id_tbfiledata" => $row_recordset["id_tbfiledata"], 
						"id_minfile" => $_REQUEST["id_minfile"], #
						"refno" => $_REQUEST["refno"], #
						"from" => $_REQUEST["from"], #
						"subject" => $_REQUEST["subject"], #
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `tb_filedata` WHERE id_tbfiledata = %s", 
			GetSQLValueString($row_recordset["id_tbfiledata"], "int")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => $row_recordset["id_tbfiledata"], 
				"metadata" => array()
			);
		} else {
			$toret = array(
