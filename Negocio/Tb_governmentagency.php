 ...
 */
$filter_field = "descriptiongovagency";
...
function findAll() {
	 ...
	 */
	$fields = array('id_govagency','descriptiongovagency');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `tb_governmentagency` $where"); 
	...
	$query_recordset = "SELECT id_govagency,descriptiongovagency FROM `tb_governmentagency` $where $order";
...
function fillsubcat()
		{
			...
			$query_select = "SELECT DISTINCT id_subgovagency,id_govagency ,descriptionsubagency
                             from tb_subgovagency  ORDER BY id_govagency ASC";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `tb_governmentagency` (id_govagency,descriptiongovagency) VALUES (%s,%s)" ,			
	        GetSQLValueString($_REQUEST["id_govagency"], "text"), # 
			GetSQLValueString($_REQUEST["descriptiongovagency"], "text")# 
	);
		...
		$toret = array(
			"data" => array(
				array(
					...
					"descriptiongovagency" => $_REQUEST["descriptiongovagency"]# 
				)
			), 
			"metadata" => array()
		);
...
function insert2() {
	...
	//build and execute the insert query
	$query_insert = sprintf("INSERT INTO `tb_subgovagency` (id_subgovagency,id_govagency,descriptionsubagency) VALUES (%s,%s,%s)" ,			
	        GetSQLValueString($_REQUEST["subcode"], "text"), # 
	        GetSQLValueString($_REQUEST["id_govagency"], "text"), # 
			GetSQLValueString($_REQUEST["descriptiongovagency"], "text")# 
	);
	$ok = mysql_query($query_insert);
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `tb_governmentagency` WHERE id_govagency = %s",
		GetSQLValueString($_REQUEST["id_govagency"], "int")
	);
	$recordset = mysql_query($query_recordset, $conn);
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `tb_governmentagency` SET descriptiongovagency = %s WHERE id_govagency = %s", 
			GetSQLValueString($_REQUEST["descriptiongovagency"], "text"), 
			GetSQLValueString($row_recordset["id_govagency"], "int")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => array(
					array(
						"id_govagency" => $row_recordset["id_govagency"], 
						"descriptiongovagency" => $_REQUEST["descriptiongovagency"]#
					)
				), 
				"metadata" => array()
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `tb_governmentagency` WHERE id_govagency = %s", 
			GetSQLValueString($row_recordset["id_govagency"], "int")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => $row_recordset["id_govagency"], 
				"metadata" => array()
			);
		} else {
			$toret = array(
