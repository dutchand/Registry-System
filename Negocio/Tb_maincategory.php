...
function findAll() {
	 ...
	 */
	$fields = array('id_category','descriptioncategory');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `tb_maincategory` $where"); 
	...
	$query_recordset = "SELECT id_category,descriptioncategory FROM `tb_maincategory` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `tb_maincategory` (id_category,descriptioncategory) VALUES (%s,%s)" ,			GetSQLValueString($_REQUEST["id_category"], "text"), # 
			GetSQLValueString($_REQUEST["descriptioncategory"], "text")# 
	);
		...
		$toret = array(
			"data" => array(
				array(
					...
					"descriptioncategory" => $_REQUEST["descriptioncategory"]# 
				)
			), 
			"metadata" => array()
		);
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `tb_maincategory` WHERE id_category = %s",
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `tb_maincategory` SET descriptioncategory = %s WHERE id_category = %s", 
			GetSQLValueString($_REQUEST["descriptioncategory"], "text"), 
			GetSQLValueString($row_recordset["id_category"], "text")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => array(
					array(
						"id_category" => $row_recordset["id_category"], 
						"descriptioncategory" => $_REQUEST["descriptioncategory"]#
					)
				), 
				"metadata" => array()
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `tb_maincategory` WHERE id_category = %s", 
			GetSQLValueString($row_recordset["id_category"], "text")
		);
		...
		if ($ok) {
			...
			$toret = array(
				"data" => $row_recordset["id_category"], 
				"metadata" => array()
			);
		} else {
			$toret = array(
